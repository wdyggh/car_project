...
typedef struct __car_info CAR_INFO {
	char id;
...
int server_parsing( unsigned char *pData ) {
	...
	switch( pData[2] ) {
		...
		case 'X': 	COMMAND_STATE = LED_OFF;
					break;		
		case 'Y': 	COMMAND_STATE = EMAGNET_ON;	
					break;	
		case 'Z': 	COMMAND_STATE = EMAGNET_OFF;	
					break;	
		default:	
					break;
	}
	...
	switch(COMMAND_STATE) {
		...
		case REQUEST:	
						switch(dir_receive){
							...
							case DRIVE: 	direction_state = DRIVE;
											drive_state = DRIVE;	// arrive my position
											debug_data ('D');		// move
											break;
